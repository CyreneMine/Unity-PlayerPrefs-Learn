...
- 涉及 API：`PlayerPrefs.SetString`、`PlayerPrefs.SetInt`、`PlayerPrefs.GetString`、`PlayerPrefs.GetInt`、`PlayerPrefs.Save`。
- 完成的练习：为玩家数据增加 key 前缀，让同一套字段可以按 `Player1`、`Player2` 等不同玩家分开保存；创建排行榜数据类，保存玩家名、分数和通关时间；在重复读取玩家装备前清空旧列表，避免重复追加。
...
- 后续注意：排行榜已经保存并展示通关时间，排序时也可以继续考虑它，例如同分时用通关时间作为比较条件；更新成绩时要注意分数和通关时间是否来自同一次通关记录。
